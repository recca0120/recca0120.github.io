...
需要強調「補充說明、小技巧、警告、關鍵重點」時用**純 blockquote**：

```markdown
> 一句話重點或補充說明
```

**避免 GitHub Alerts**（`> [!NOTE]` / `> [!TIP]` 等）。主站 Stack theme 會渲染成彩色框，但 dev.to 和 Hashnode cross-post 不支援，會把 `[!NOTE]` 字面留在文章裡，很醜。
...
讓 blockquote 的**內容本身**清楚表達語氣（「注意」「提醒」「小技巧」等關鍵字），不靠視覺標籤區分。
